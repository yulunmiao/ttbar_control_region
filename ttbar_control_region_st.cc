...
bool fill_histogram(TH1F* hist,const float cross_section,const char* path){
	...
	for(unsigned int i=0;i<chain->GetEntries();i++){
		chain->GetEntry(i);
		float result=0;
		for(unsigned int j=0;j<lepp4->size();j++){
			result+=lepp4->at(j).pt();
		}
		for(unsigned int j=0;j<fatjetp4->size();j++){
			result+=fatjetp4->at(j).pt();
		}
		for(unsigned int j=0;j<jetp4->size();j++){
			bool overlap=false;
			for(unsigned int k=0;k<fatjetp4->size();k++){
				if(ROOT::Math::VectorUtil::DeltaR(jetp4->at(j),fatjetp4->at(k))<0.8){
					overlap=true;break;	
				}
			}
