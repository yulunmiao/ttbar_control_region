...
bool fill_histogram(TH1F* hist,const float cross_section,const char* path){
	...
	for(unsigned int i=0;i<chain->GetEntries();i++){
		chain->GetEntry(i);
		hist->Fill((lepp4->at(0)+lepp4->at(1)).M(),scale);
	}
	return true;
}
...
int ttbar_control_region_leppairmass(){
        gStyle->SetOptStat(0);
	...
	vector<LV> *lepp4=0;
        ...
        TH1F *hdata=new TH1F("",";m_{ll}(GeV);N/Events",NBIN,INF,SUP);
        ...
        for(unsigned int i=0;i<data->GetEntries();i++){
                ...
                hdata->Fill((lepp4->at(0)+lepp4->at(1)).M());
        }
